...
> **Discuss:** Why would only looking at the previous character be insufficient for predicting the next word in a language like English? What kinds of long-range dependencies would a bigger context window capture?
...
![Full GPT architecture. The transformer block (center) repeats $N_x$ times. Each block contains multi-head self-attention, residual connections, layer norm, and a feed-forward network. Source: [Daniel Dugas, *GPT Architecture*](https://dugas.ch/artificial_curiosity/GPT_architecture.html)](images/fullarch-gpt-dugas-ch.png)
