...
- **notebook-02** — trained a tiny GPT on DNA; generated DNA-looking sequences
- **This deck** — what happens when you scale that exact idea to the whole tree of life
...
| **Training data** | 1 chromosome | 9.3T nucleotides |
...
To model gene regulation (enhancers hundreds of kb from their target genes), you need 1M+ context. A pure transformer can't get there.

---

## Biology's multi-scale structure
...
Evo 2 assigns a **likelihood** to every DNA sequence. A lower score for a mutant → the model predicts the mutation disrupts function.

**No fine-tuning. No labeled examples.**
...
**Why this matters**: most clinical variants are rare and unlabeled. A zero-shot model can prioritize them immediately.
:::
:::
...
To look inside the model, researchers trained a **Sparse Autoencoder (SAE)** on layer 26 activations — no labels, no supervision.
...
2. **Architecture matters at genome scale** — StripedHyena 2 replaces most attention with fast convolutions to reach 1M-token context

3. **Zero-shot = useful** — likelihood scores predict variant pathogenicity without any task-specific training
