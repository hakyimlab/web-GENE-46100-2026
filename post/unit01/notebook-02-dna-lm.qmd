---
title: "02 · Genomic Language Model with nanoGPT"
author: "Henry Raeder"
date: '2026-04-25'
categories:
  - notebook
description: "Pretrain a character-level GPT on the human reference genome, then fine-tune for promoter activity and TF binding prediction."
...
Let's talk about Attention. The first paper which outlines the Transformer neural network architecture (which relies heavily on the Attention mechanism and is the basis for most large language models today) is [Attention is All you Need](https://proceedings.neurips.cc/paper_files/paper/2017/file/3f5ee243547dee91fbd053c1c4a845aa-Paper.pdf). This paper describes the application of Attention in a machine translation model, but the underlying ideas can be applied across any model that processes sequences of text.
...
1. Go to the [course GitHub repository](https://github.com/hakyimlab/web-GENE-46100-2026) and download the four files inside `post/unit01/data/`:
   - `promoter_all_train.parquet`
   - `promoter_all_test.parquet`
   - `enhancers_types_train.parquet`
   - `enhancers_types_test.parquet`
2. In the Colab file browser (left sidebar → folder icon), create a folder named `data` in the current working directory.
3. Upload all four files into that `data/` folder.
...
import os; os.symlink("web-GENE-46100-2026/post/unit01/data", "data")
```
:::

```{python}
# Load the promoter dataset from local parquet files
data_dir = "data"
...
if os.path.exists(_enhancer_ckpt) and not force_retrain_enhancer:
    ...
    print(f"Enhancer model saved to {_enhancer_ckpt}")
```
